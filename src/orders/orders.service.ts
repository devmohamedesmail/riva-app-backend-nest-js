...
import { OrderNotificationService } from '../notifications/order-notification/order-notification.service';
...
export class OrdersService {
    ...
    async createGroupOrder(dto: CreateGroupOrderDto) {
        ...
        /**
          * Run notifications & email in background
        */
        setImmediate(async () => {
            try {
                if (user_id) {
                    await Promise.all(
                        result.orders.map(
                            (order) =>
                                this.orderNotificationService.sendOrderNotification(
                                    order,
                                    user_id,
                                    customer_name,
                                    order.store_id,
                    ...
                    console.log(
                        'Socket notifications sent successfully',
                    );
                }
            } catch (error) {
                console.error(
